...
# 从环境变量读取 GitHub Secrets 中的远程链接
input_urls = os.getenv('INPUT_URLS', '')
# 加载 links 列表，必须从远程链接获取
links = []
if input_urls:
    try:
        response = requests.get(input_urls, headers={'User-Agent': 'Clash Verge/1.7.7'}, verify=False)
        if response.status_code == 200:
            content = response.text
            links = [link.strip() for link in content.splitlines() if link.strip()]
            print(f"成功从 {input_urls} 下载链接列表，共 {len(links)} 个链接。")
        else:
            print(f"无法从 {input_urls} 下载链接列表，状态码: {response.status_code}")
            sys.exit(1)  # 退出程序，不回退到任何默认值
    except Exception as e:
        print(f"从 {input_urls} 下载链接列表时出错: {e}")
        sys.exit(1)  # 退出程序，不回退到任何默认值
else:
    print("错误: 环境变量 INPUT_URLS 未设置，无法获取链接列表。")
    sys.exit(1)  # 退出程序，不回退到任何默认值
# Clash 配置文件的基础结构
